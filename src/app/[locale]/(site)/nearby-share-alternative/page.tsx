 ...
 * all 13 locales via the "nearbyShareAlt" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "nearbyShareAlt" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/nearby-share-alternative"),
    ...sharedOpenGraph(title, description, "/nearby-share-alternative"),
  };
}
...
export default async function NearbyShareAlternativePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("nearbyShareAlt");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/nearby-share-alternative" />
      </main>
