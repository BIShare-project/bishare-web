 ...
 * all 13 locales via the "pcToPhone" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "pcToPhone" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/transfer-files-pc-to-phone"),
    ...sharedOpenGraph(title, description, "/transfer-files-pc-to-phone"),
  };
}
...
export default async function TransferFilesPcToPhonePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("pcToPhone");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/transfer-files-pc-to-phone" />
      </main>
