...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "pcToPc" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/transfer-files-between-computers"),
    ...sharedOpenGraph(title, description, "/transfer-files-between-computers"),
  };
}

/* Columns: USB drive · Cloud drive · BIShare (labels via comparison.cols). */
const COMPARISON: Array<{
  ...
  { id: "r2", airdrop: false, quick: true, bishare: true },
...
export default async function TransferFilesBetweenComputersPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("pcToPc");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/transfer-files-between-computers" />
      </main>
