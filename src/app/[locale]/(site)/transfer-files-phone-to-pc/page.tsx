 ...
 * all 13 locales via the "phoneToPc" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "phoneToPc" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/transfer-files-phone-to-pc"),
    ...sharedOpenGraph(title, description, "/transfer-files-phone-to-pc"),
  };
}
...
export default async function TransferFilesPhoneToPcPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("phoneToPc");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-6 py-16 md:py-24">
        ...
        <RelatedLinks current="/transfer-files-phone-to-pc" />
      </main>
