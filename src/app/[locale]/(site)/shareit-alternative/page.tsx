 ...
 * all 13 locales via the "shareitAlt" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "shareitAlt" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/shareit-alternative"),
    ...sharedOpenGraph(title, description, "/shareit-alternative"),
  };
}
...
export default async function ShareitAlternativePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("shareitAlt");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/shareit-alternative" />
      </main>
