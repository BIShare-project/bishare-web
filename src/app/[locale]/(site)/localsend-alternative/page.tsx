...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "localsendAlt" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/localsend-alternative"),
    ...sharedOpenGraph(title, description, "/localsend-alternative"),
  };
}

/* Truth table for the comparison — labels come from the namespace.
   Columns: LocalSend · Snapdrop · BIShare. */
const COMPARISON: Array<{
  ...
  { id: "r0", localsend: true, snapdrop: true, bishare: true },
  { id: "r1", localsend: true, snapdrop: true, bishare: true },
  { id: "r2", localsend: false, snapdrop: false, bishare: true },
  { id: "r3", localsend: false, snapdrop: true, bishare: true },
  { id: "r4", localsend: false, snapdrop: false, bishare: true },
];
...
export default async function LocalSendAlternativePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("localsendAlt");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
              ...
              <thead>
                <tr className="border-b border-border bg-background-raised text-muted-foreground">
                  ...
                  <th className="px-4 py-3 font-medium">
                    {t("comparison.cols.localsend")}
                  </th>
                  <th className="px-4 py-3 font-medium">
                    {t("comparison.cols.snapdrop")}
                  </th>
                ...
                {COMPARISON.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Cell ok={r.localsend} yes={yes} no={no} />
                    </td>
                    <td className="px-4 py-3">
                      <Cell ok={r.snapdrop} yes={yes} no={no} />
                    </td>
                    <td className="px-4 py-3">
        ...
        <RelatedLinks current="/localsend-alternative" />
      </main>
