 ...
 * all 13 locales via the "sharedropAlt" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "sharedropAlt" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/sharedrop-alternative"),
    ...sharedOpenGraph(title, description, "/sharedrop-alternative"),
  };
}
...
export default async function SharedropAlternativePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("sharedropAlt");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/sharedrop-alternative" />
      </main>
