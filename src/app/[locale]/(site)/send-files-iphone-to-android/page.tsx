 ...
 * all 13 locales via the "iphoneToAndroid" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "iphoneToAndroid" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/send-files-iphone-to-android"),
    ...sharedOpenGraph(title, description, "/send-files-iphone-to-android"),
  };
}
...
export default async function SendFilesIphoneToAndroidPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("iphoneToAndroid");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/send-files-iphone-to-android" />
      </main>
