 ...
 * all 13 locales via the "androidToIphone" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "androidToIphone" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/send-files-android-to-iphone"),
    ...sharedOpenGraph(title, description, "/send-files-android-to-iphone"),
  };
}
...
export default async function SendFilesAndroidToIphonePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("androidToIphone");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/send-files-android-to-iphone" />
      </main>
