 ...
 * all 13 locales via the "wetransferAlt" namespace, with FAQPage structured
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "wetransferAlt" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/wetransfer-alternative"),
    ...sharedOpenGraph(title, description, "/wetransfer-alternative"),
  };
}
...
export default async function WetransferAlternativePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("wetransferAlt");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-6 py-16 md:py-24">
        ...
        <RelatedLinks current="/wetransfer-alternative" />
      </main>
