...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "airdropAlt" });
  const title = t("meta.title");
  const description = t("meta.description");
  return {
    title: { absolute: title },
    description,
    alternates: buildAlternates(locale, "/airdrop-alternative"),
    ...sharedOpenGraph(title, description, "/airdrop-alternative"),
  };
}
...
export default async function AirdropAlternativePage({
  ...
  const t = await getTranslations("airdropAlt");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6 md:py-24">
        ...
        <RelatedLinks current="/airdrop-alternative" />
      </main>
